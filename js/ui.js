  ...
  card.innerHTML = `
    ...
    <div class="card-body">
      ...
      <div class="card-share">
        <a href="${montarLinkShareWpp(produto)}" target="_blank" rel="noopener"
           class="share-btn wpp" title="Compartilhar no WhatsApp" onclick="event.stopPropagation()">
        ...
        <button class="share-btn copy" title="Copiar link para Instagram" data-produto-id="${produto.id}">
...
/* ─── SHARE ──────────────────────────────────────────────── */
const BASE_URL = 'https://rabsouza.github.io/bazar_emicida/produtos.html';
const _produtosMap = {};

function urlProduto(produto) {
  return `${BASE_URL}?id=${produto.id}`;
}
...
function copiarLink(produto, btn) {
  navigator.clipboard.writeText(urlProduto(produto)).then(() => {
    btn.classList.add('copiado');
    btn.querySelector('span').textContent = 'Copiado!';
    showToast('Link copiado — cola no Instagram Stories 🔗');
    setTimeout(() => {
